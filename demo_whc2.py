...
class WHC(AbstractModel):
    def __init__(
        ...
        *,
        ...
        # まずはデフォルト値
        self.sequence_mode: str = "2d"  # "2d", "lstm", "3dcnn"
        self.sequence_len: int = 1

        # 入力テンソル形状から、シーケンス長とモードを推定
        ...
        # シーケンスモデルなのに長さが1以下なら、最低 4 フレームにする
        if self.sequence_mode != "2d" and self.sequence_len <= 1:
            self.sequence_len = 4

        # h,w インデックスを更新した後で、入力サイズを決定
    ...
    def __call__(self, image: Any) -> float:
        """
        image:
          - 2D モード: 1 枚のフレーム (H, W, C)
          - シーケンスモード: フレームの list/tuple [frame_t, ...]
        """
        if image is None:
        ...
        if self.sequence_mode == "2d":
            ...
            if self.sequence_mode == "3dcnn":
                ...
                # (N, C, T, H, W)
                inference_image = np.asarray([stacked], dtype=self._input_dtypes[0])
            else:
                # LSTM 系: (N, T, C, H, W)
                inference_image = np.asarray([stacked], dtype=self._input_dtypes[0])
    ...
    while True:
        ...
        for box in target_boxes:
            ...
            try:
                ...
                box.hand_state = 1 if prob_waving >= 0.10 else 0
            #except Exception:
            #    continue
            except Exception as e:
                import traceback
                print("=== WHC ERROR ===")
                print("track_id:", box.track_id)
                print("sequence_mode:", whc_classifier.sequence_mode,
              "sequence_len:", whc_classifier.sequence_len)
                traceback.print_exc()
                continue
